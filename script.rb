def caeser_cipher(string, shift) 
  ...
  shiftedArray = codepointsArray.map do |codepoint|
    if codepoint < 65 || codepoint > 122 then return codepoint # although there is no fundamental issue in doing so, do not modify the character if not in the latin alphabet
